...
本节用于对齐旧主题分析和当前 Go 版代码状态。下面列出的能力已经在 `D:\codex\sakurairo-go` 中实现，后续阅读本文的迁移建议时，应优先以本节为准。

已实现或基本实现：

- 公开页面：主页、文章详情、普通归档 `/archive`、月度归档 `/archives`、搜索、分类、标签、友情链接 `/links`、说说/短动态 `/moments`、独立页面 `/page/{slug}`、404/500 软风格错误页。
...
- 评论系统：公开提交、蜜罐字段、轻量垃圾过滤、Markdown 渲染与 HTML sanitize、嵌套回复、私密评论、后台审核/删除/私密切换、后台批量评论操作、新评论管理员通知、父评论作者 opt-in 回复邮件通知。
- 站点设置：标题、副标题、Profile Name/Profile Avatar、公告、导航、社交链接、首页 hero、overlay 透明度、默认文章封面、页脚文案、Focus Cards、sakura 特效。
- 阅读与发现：浏览量、点赞、上一篇/下一篇、作者卡、自动文章/页面目录、可配置版权声明、可配置分享区、打赏面板、复制来源提示、实时搜索 overlay、搜索索引 API、随机封面/特色图 API、Atom feed、sitemap、robots.txt、SEO/Open Graph/Twitter metadata。
- 媒体库：本地上传、详情编辑、搜索/筛选、批量删除、文章/页面编辑器媒体选择器。
- 部署与运维：Git-based deploy script、本地/远端部署锁、健康检查 build metadata、请求日志、SMTP 配置文档。

当前仍值得完善的功能：

- 评论表情兼容、评论图片插入/上传。
- 评论审核体验：更明确的 review states、垃圾评论隔离与恢复。
...
现有 Go 版已覆盖文章、独立页面、分类、标签、评论、友情链接、媒体库、说说/短动态和基础后台。用户资料目前按单用户个人站处理，主要通过 `site_settings` 中的 Profile Name/Profile Avatar 承载；完整多用户资料和前台注册仍可暂缓。
...
- 当前 Go 版已实现 `moments` 表、公开 `/moments` 页面和 `/admin/moments` 后台管理。
...
- 归档页、友情链接、说说和独立页面已经在当前 Go 版实现，应继续保持。
...
- 更完整的评论审核工作流，包括 review states、垃圾评论隔离与恢复。
